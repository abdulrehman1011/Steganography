package DataStoneWall.Steganography;
...
/**
 * <p>
 * Class that handles all the Haar transformation and recovery calculations for
 * the HaarDemo front-end class. The Haar Transformation employed for images
 * with dimensions of (2^x, 2^x) is a typical Haar generateTransformImages.
 * Images without equal sides or sides that are not a power of two, are handled
 * by my custom Haar Transform which provides for encoding of the last remaining
 * single pixel that occurs in various stages of the generateTransformImages
 * when the sides are not a power of two.
 *
 * <p>
 * These leftover pixels are handled by adding the previous pixel to the
 * leftover (unencoded/original) pixel and dividing by two. Then subtracting the
 * result of the previous calculation from the leftover pixel and storing the
 * result in the last spot of the transformation area. Recovering these pixels
 * is done by adding two times the stored value for the leftover pixel to the
 * recovered value of the previous pixel.
 *
 * <p>
 * Provides both basic Haar generateTransformImages images and expanded images
 * of the averaged portion of each step to demonstrate the degradation of the
 * image quality.
 *
 * <p>
 * Provides the ability to generate image files for each stage of the
 * transformation.
 *
 * @author Jason Gould
 */
public class HaarTransform {
    ...
    int transformRequired = 1;

    /**
     * Converts an image file to a three dimensional array where the first
     * dimension equates to a numOfBands in the image, and the other two
     * dimensions form the 2D matrix of pixels
     *
    ...
    public void loadImage(BufferedImage inputImage){
    ...
    /**
     * Transforms the last image that has been converted to a 3D array by this
     * object Determines how many row and column transforms are needed to
     * generateTransformImages the image via Haar transformation until one
     * averaged pixel remains and the rest of the image is coefficients from the
     * transformation given the image size. Then calls the performRowTransform
     * and performColumnTransform methods the requisite number of times to
     * restore the image. Also calls generateImage and convert
     * array2ImageExpansion after each transformation phase. //* @param imgList
     * ArrayList to store the images from the generateTransformImages
     *
     * @param genFiles Flag to control the generation of files for the
     * generateTransformImages stages
     */
...
//            }
            transformsPerformed++;
            System.out.println(resultImages.size() + "----" + transformsPerformed);
    ...
    public BufferedImage generateTransformImg(BufferedImage result) {
         ArrayList<BufferedImage> resultImages = new ArrayList<>();
            for (int i = 0; i< transformRequired; i++)
            {
                performRowTransform(i);
                performColumnTransform(i);
            }
    ...
    public BufferedImage generateInverseTransformImg() {
        generateImage();
        return generatePixelExpansionImage(0, 0);
                ...
                for (int currSubImageRow = numOfSubImageRows - ZERO_INDEX_OFFSET; currSubImageRow >= 0; currSubImageRow--) {
                    for (int currColumnOffset = 0; currColumnOffset < columnScalingFactor; currColumnOffset++) {
                        for (int currRowOffset = 0; currRowOffset < rowScalingFactor; currRowOffset++) {
                            result3DArray[(currSubImageColumn * columnScalingFactor) + currColumnOffset][(currSubImageRow * rowScalingFactor) + currRowOffset][bandIndex]
    ...
    protected void createImageFilesForAllStages(ArrayList<BufferedImage> images, String processThatCreatedImage) {
        ...
        for (BufferedImage image : images) {
            if (isStandardImage(image, images)) {
                System.out.println(numOfImageInSequence + "###");
    ...
    ///////////////////////////////////////////////////////////////////
    /**
     * Recovers the last image that has undergone an Haar Transform by this
     * object. Determines how many row and column transforms are needed to
     * generateRecoverImages the image if the image has been reduced to one
     * averaged pixel by Haar transformation from the original image given the
     * image size. Then calls the performColumnRecover and performRowRecover
     * methods the requisite number of times to restore the image. Also calls
     * generateImage and convert array2ImageExpansion after each recovery phase.
     * //* @param imgList ArrayList to store the images from the
     * generateTransformImages
     *
     * @param genFiles Flag to control the generation of files for the
     * generateTransformImages stages
     */
        ...
        for (boolean columnRecoverPerformed = false, rowRecoverPerformed = false;
            ...
            if (columnRecoversNeeded > 0 && columnRecoversNeeded >= rowRecoversNeeded) {
                if (columnRecoversNeeded > rowRecoversNeeded) {
                    ...
                    System.out.println("Col Trans " + rowRecoversNeeded + "---" + columnRecoversNeeded);
            ...
            if (rowRecoversNeeded > 0 && rowRecoversNeeded >= columnRecoversNeeded) {
                if (rowRecoversNeeded > columnRecoversNeeded) {
                    ...
                    System.out.println("Row Trans " + rowRecoversNeeded + "---" + columnRecoversNeeded);
                    rowRecoverPerformed = true;
                }
            }
            //updateProgress(worker, progressBar, progress+= progressIncrementStep);
    ...
    public BufferedImage recoverTransformImage(BufferedImage image)
    {
         ArrayList<BufferedImage> resultImages = new ArrayList<>();
            for (int i = transformRequired; i > 0; i--)
            {
                performColumnRecover(i,i);
                performRowRecover(i,i);
            }
            image = generateImage();
    ...
    /**
     * Performs the Haar recovery functions on the designated rows of the
     * section of the transformed image
     *
     * @param rowRecoversRemaining The number of recovery steps left to perform
     * on the rows of the image
     * @param columnRecoversRemaining The number of recovery steps left to
     * perform on the columns of the image //* @param even Flag indicating if
     * the current length of the row segment to be transformed is even.
     */
    ...
    /**
     * Performs the Haar recovery functions on the columns of the designated
     * section of the transformed image
     *
     * @param rowRecoversRemaining The number of recovery steps left to perform
     * on the rows of the image
     * @param columnRecoversRemaining The number of recovery steps left to
     * perform on the columns of the image //* @param even Flag indicating if
     * the current length of the column segment to be transformed is even.
     */
